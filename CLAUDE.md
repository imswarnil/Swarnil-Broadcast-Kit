...
`CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID`, and it does not have them
today.** Without them the site is built and checked, the deploy step is skipped,
and the job still reports success — so `deploy: success` in a run listing does
**not** mean the site changed. The run summary now says so in words, and the job
raises a warning. Until the secrets exist, publishing is `npx wrangler@4 deploy`
by hand, and a change is not live until you have curled the new URL.
